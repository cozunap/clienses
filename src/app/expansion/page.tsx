...
export default function ExpansionPage() {
    ...
    const s = dict.expansion;

    return (
        <main className="min-h-screen bg-bg-light">
            <Navbar />

            {/* Hero Section - Escalamiento */}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-32 items-center">
                        <Reveal className="space-y-16">
                            ...
                            <div className="space-y-10 text-xl text-structure/60 leading-relaxed font-light">
                                <p>
                                    {s.content_desc}<strong>{s.content_desc_bold}</strong>{s.content_desc_mid}<span className="text-primary font-bold">{s.content_desc_accent}</span>{s.content_desc_end}<span className="text-primary font-bold">{s.content_desc_markets}</span>
                                </p>
                                <ul className="space-y-6">
                                    {s.items.map((item: string) => (
                                        <li key={item} className="flex items-center gap-6 group">
                                            ...
                                            <span className="font-bold text-structure uppercase text-sm group-hover:text-primary transition-colors">{item}</span>
                                        </li>
                                    ))}
                                </ul>
                            </div>
